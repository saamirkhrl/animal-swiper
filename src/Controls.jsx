...
function Controls() {
  ...
  return (
    <div className="container">
      <h2 className="animal-tinder-name">Animal Swiper</h2>
      <div className="buttons">
        <button className="btn yes" onClick={next}>
          👍
        </button>
        <button className="btn no" onClick={next}>
          👎
        </button>
      </div>
      <div className="image-container">
        <img
          className={`picture ${animateOut ? "animate-out" : ""}`}
          src={pictureUrl}
          alt="Animal"
        />
        {nextPictureUrl && (
          <img
            className={`picture next ${!animateOut ? "animate-in" : ""}`}
            src={nextPictureUrl}
            alt="Animal"
          />
        )}
